...
class activity_report(models.Model):
    ...
    def get_activity_detail(self,date_start,date_end,room_no):
        self._cr.execute(
            "select hh.current_date," \
            "ppt.name as Activity, "\
            "pt.name as Room," \
            "rs.login, " \
            "hha.clean_start_time, " \
            "hha.clean_end_time, " \
            "(hha.clean_end_time-hha.clean_start_time) as duration " \
            "from hotel_housekeeping as hh" \
            ...
            "where hh.current_date >= %s " \
            "and hh.current_date <= %s  " \
            "and hh.room_no=pp.id " \
            "and pp.name_template=%s ", (date_start,date_end,str(room_no))) 
